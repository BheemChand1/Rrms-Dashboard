  ...
  return (
    <>
      ...
      >
        ...
        <button 
          className="hidden lg:flex absolute -right-3 top-20 z-50 w-6 h-6 bg-primary text-white rounded-full items-center justify-center shadow-lg hover:bg-primary/80 transition-colors"
          onClick={onToggleCollapse}
        >
          <ChevronLeft className={`h-4 w-4 transition-transform duration-300 ${isCollapsed ? "rotate-180" : ""}`} />
        </button>

        <div className={`p-4 flex justify-center border-b border-sidebar-border/50 ${isCollapsed ? "px-2" : ""}`}>
          <div className={`bg-white/10 rounded-2xl backdrop-blur-sm ${isCollapsed ? "p-1.5" : "p-2"}`}>
            <img 
              src={railwayLogo} 
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2" style={{ scrollbarWidth: 'thin', scrollbarColor: 'hsl(var(--sidebar-accent)) transparent' }}>
          {menuItems.map((item, index) => (
            <a
              ...
              className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} px-3 py-2.5 text-sm rounded-xl mb-1 transition-all duration-200 group ${
                item.active 
            ...
            >
              ...
              {!isCollapsed && item.hasSubmenu && <ChevronRight className="h-4 w-4 opacity-50" />}
            </a>
          ))}
        </nav>
