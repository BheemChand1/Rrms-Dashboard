  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-2 xl:py-3 px-2 xl:px-3" style={{ scrollbarWidth: 'thin', scrollbarColor: 'hsl(var(--sidebar-accent)) transparent' }}>
          {menuItems.map((item, index) => (
            ...
            >
              <div className={`flex items-center ${isCollapsed ? "" : "gap-2.5 xl:gap-3"}`}>
                <item.icon className={`h-4 w-4 xl:h-5 xl:w-5 ${item.active ? '' : 'group-hover:scale-110'} transition-transform`} />
                {!isCollapsed && <span className="font-medium">{item.label}</span>}
              </div>
              {!isCollapsed && item.hasSubmenu && <ChevronRight className="h-3.5 w-3.5 xl:h-4 xl:w-4 opacity-50" />}
            </a>
          ))}
        </nav>
