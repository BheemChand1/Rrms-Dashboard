  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-3 xl:py-4 px-2 xl:px-3" style={{ scrollbarWidth: 'thin', scrollbarColor: 'hsl(var(--sidebar-accent)) transparent' }}>
          {menuItems.map((item, index) => (
            <a
              key={index}
              href="#"
              title={isCollapsed ? item.label : undefined}
              className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} px-3 xl:px-4 py-2.5 xl:py-3 2xl:py-3.5 text-sm xl:text-base 2xl:text-lg rounded-xl mb-1 xl:mb-1.5 transition-all duration-200 group ${
                item.active 
                  ? "bg-primary text-white shadow-lg shadow-primary/30" 
                  : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-white"
              }`}
            >
              <div className={`flex items-center ${isCollapsed ? "" : "gap-3 xl:gap-4"}`}>
                <item.icon className={`h-4 w-4 xl:h-5 xl:w-5 2xl:h-6 2xl:w-6 ${item.active ? '' : 'group-hover:scale-110'} transition-transform`} />
                {!isCollapsed && <span className="font-medium">{item.label}</span>}
              </div>
              {!isCollapsed && item.hasSubmenu && <ChevronRight className="h-4 w-4 xl:h-5 xl:w-5 opacity-50" />}
            </a>
          ))}
        </nav>
